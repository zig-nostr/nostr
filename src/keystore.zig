...
//!
//! A signer should never persist the user's secret key in plaintext. This
//! module wraps `nip49` to turn a 32-byte secret key into an `ncryptsec1...`
//! blob (scrypt + XChaCha20-Poly1305) and back, and to read and write that blob
//! as a `0600` key file. It is the at-rest half of running a NIP-46 signer (the
//! serving half is `signer.zig`); a client that only talks to a bunker does not
//! need it.
//!
...
// ---------------------------------------------------------------------------
// Tests — exercise the file surface end to end against a real temp dir:
